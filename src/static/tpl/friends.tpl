<div class="userlist">
  <article class="userlist__item animation-top-1">
    <div class="userlist__avatar">
      <div class="avatar">
        <img class="avatar__img" src="img/1.jpg" alt="">
      </div>
      <div class="userlist__online"></div>
    </div>
    <div class="userlist__content">
      <div class="userlist__name">Elisabetta  R.</div>
      <time class="userlist__time">5:45 am</time>
      <p class="userlist__mess">Lorem, ipsum dolor sit amet.</p>
    </div>
  </article>

  <article class="userlist__item animation-top-2">
    <div class="userlist__avatar">
      <div class="avatar">
        <img class="avatar__img" src="img/2.jpg" alt="">
      </div>
      <div class="userlist__online userlist__online--danger"></div>
    </div>
    <div class="userlist__content">
      <div class="userlist__name">Donald T.</div>
      <time class="userlist__time">3:44 pm</time>
      <p class="userlist__mess">Est reprehenderit voluptate.</p>
    </div>
  </article>

  <article class="userlist__item animation-top-3">
    <div class="userlist__avatar">
      <div class="avatar">
        <img class="avatar__img" src="img/3.jpg" alt="">
      </div>
      <div class="userlist__online userlist__online--grey"></div>
    </div>
    <div class="userlist__content">
      <div class="userlist__name">Miuki S.</div>
      <time class="userlist__time">6:23 am</time>
      <p class="userlist__mess">Corrupti alias molestiae suscipit.</p>
    </div>
  </article>

  <article class="userlist__item animation-top-4">
    <div class="userlist__avatar">
      <div class="avatar">
        <img class="avatar__img" src="img/4.jpg" alt="">
      </div>
      <div class="userlist__online"></div>
    </div>
    <div class="userlist__content">
      <div class="userlist__name">Katerina A.</div>
      <time class="userlist__time">7:11 am</time>
      <p class="userlist__mess">Tenetur asperiores nemo repudiandae.</p>
    </div>
  </article>

  <article class="userlist__item animation-top-5">
    <div class="userlist__avatar">
      <div class="avatar">
        <img class="avatar__img" src="img/5.jpg" alt="">
      </div>
      <div class="userlist__online"></div>
    </div>
    <div class="userlist__content">
      <div class="userlist__name">Vladimir P.</div>
      <time class="userlist__time">11:53 pm</time>
      <p class="userlist__mess">Autem necessitatibus natus similique!</p>
    </div>
  </article>
</div>